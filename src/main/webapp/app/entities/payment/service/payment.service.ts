...
export class PaymentService {
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    return Object.assign({}, payment, {
      datePaid: payment.datePaid?.isValid() ? payment.datePaid.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.datePaid = res.body.datePaid ? dayjs(res.body.datePaid) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        payment.datePaid = payment.datePaid ? dayjs(payment.datePaid) : undefined;
      });
    }
    return res;
  }
