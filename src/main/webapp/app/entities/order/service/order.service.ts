...
export class OrderService {
  ...
  protected convertDateFromClient(order: IOrder): IOrder {
    return Object.assign({}, order, {
      dateOrdered: order.dateOrdered?.isValid() ? order.dateOrdered.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateOrdered = res.body.dateOrdered ? dayjs(res.body.dateOrdered) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((order: IOrder) => {
        order.dateOrdered = order.dateOrdered ? dayjs(order.dateOrdered) : undefined;
      });
    }
    return res;
  }
